...
export const Trip = () => {
    ...
    return (
      <div className="flex h-full min-h-[calc(100vh-64px)] w-full flex-col gap-6 p-8">
        <Empty>
          ...
          <EmptyContent>
            <Button asChild>
              <Link href="/trips">{t('details.empty.backToTrips')}</Link>
            </Button>
          </EmptyContent>
        </Empty>
      </div>
  ...
  if (tripData.status === TripStatus.DELETED) {
    return (
      <div className="flex h-full min-h-[calc(100vh-64px)] w-full flex-col gap-6 p-8">
        <Empty>
          <EmptyHeader>
            ...
            <EmptyTitle>{t('details.deleted.title')}</EmptyTitle>
            <EmptyDescription>{t('details.deleted.description')}</EmptyDescription>
          </EmptyHeader>
          <EmptyContent>
            <Button asChild>
              <Link href="/trips">{t('details.empty.backToTrips')}</Link>
    ...
    <div className="flex h-full w-full flex-col gap-6">
      <div className="flex flex-col gap-4 py-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-col gap-1 sm:flex-1">
          ...
          <p className="text-primary text-sm sm:text-base">{t('details.description')}</p>
        </div>
        <div className="mt-2 flex flex-wrap gap-2 sm:mt-0">
          <UpdateTrip tripId={tripData?.slug} />
          {tripData.status !== TripStatus.DELETED && <DeleteTripDialog tripId={tripData?.slug} />}
        </div>
      </div>
      ...
      <Card className="w-full">
        <CardContent className="flex flex-col gap-6 p-6 lg:flex-row lg:items-center lg:justify-between lg:gap-10">
          <div className="flex flex-col gap-1">
            <span className="text-nowrap text-muted-foreground text-xs uppercase">{t('details.tripName')}</span>
          ...
          <div className="flex flex-col gap-1">
            <span className="text-nowrap text-muted-foreground text-xs uppercase">{t('details.busDriver')}</span>
            <span className="font-medium text-base">{tripData?.driver.user.fullName}</span>
          </div>

          <div className="hidden h-10 w-px bg-border lg:block" />

          <div className="flex flex-col gap-1">
            <span className="text-nowrap text-muted-foreground text-xs uppercase">{t('details.busName')}</span>
          ...
          <div className="flex flex-col gap-1">
            <span className="text-nowrap text-muted-foreground text-xs uppercase">{t('details.availableSeats')}</span>
            <span className="font-medium text-base">{tripData?.bus.maxPlaces - tripData?.bookings.length}</span>
          </div>

          <div className="hidden h-10 w-px bg-border lg:block" />

          <div className="flex flex-col gap-1">
            <span className="text-nowrap text-muted-foreground text-xs uppercase">{t('details.createdAt')}</span>
          ...
          <div className="flex flex-col gap-1">
            <span className="text-nowrap text-muted-foreground text-xs uppercase">{t('details.status')}</span>
        ...
        <CardContent className="flex w-full items-center gap-4 overflow-x-auto">
          <div className="relative flex w-full flex-row gap-0 pb-4">
            <div className="flex min-w-full items-start px-2">
              {tripData?.stations.map((station: { id: string; name: string; departureTime: string }, index: number) => {
      ...
      <div className="w-full">
        <TripBookings bookings={tripData?.bookings || []} />
      </div>
    </div>
  );
};
