...
import { UpdateBus } from '@/components/update-bus';
    ...
    return (
      <div className="flex h-full min-h-[calc(100vh-64px)] w-full flex-col gap-6 p-8">
        <Empty>
          ...
          <EmptyContent>
            <Button asChild>
              <Link href="/buses">{t('details.empty.backToBuses')}</Link>
            </Button>
          </EmptyContent>
        </Empty>
      </div>
        ...
        <Card className="flex-2">
          <CardHeader className="flex flex-row items-center justify-between gap-2 border-b text-lg">
            <div className="flex items-center gap-2">
              ...
              <span>{t('details.infoTitle')}</span>
            </div>
          </CardHeader>
          <CardContent className="grid grid-cols-1 gap-6 p-6 sm:grid-cols-2 lg:grid-cols-3">
            <div className="flex flex-col gap-2">
              <p className="text-muted-foreground">{t('details.name')}</p>
              <p className="truncate font-semibold text-lg">{bus.title}</p>
            </div>
            <div className="flex flex-col gap-2">
              <p className="text-muted-foreground">{t('details.licensePlate')}</p>
              <p className="truncate font-semibold text-lg">{bus.licensePlate}</p>
            </div>
            <div className="flex flex-col gap-2">
              <p className="text-muted-foreground">{t('details.maxPlaces')}</p>
            ...
            <div className="flex flex-col gap-2">
              ...
              <p className="truncate font-semibold text-lg">
                {bus.seatReservationType === 'SEAT_RESERVATION'
                  ? t('details.seatReservation')
                  : t('details.noSeatReservation')}
              </p>
            </div>
            <div className="flex flex-col gap-2">
        ...
        <Card className="h-fit flex-2 rounded-2xl shadow">
          <CardHeader className="flex flex-row items-center justify-between gap-2 border-b text-lg">
            <div className="flex items-center gap-2">
              ...
              <span>{t('details.currentTripDetails')}</span>
            </div>
            {Boolean(currentTrip) && (
              <Button asChild size="sm" variant="link">
                <Link href={`/trips/${currentTrip?.id}`}>{t('details.viewDetails')}</Link>
              </Button>
            )}
          </CardHeader>
            ...
            {currentTrip ? (
              ...
              <Empty className="py-8">
                <EmptyHeader>
                  ...
                  <EmptyDescription>{t('details.noActiveTripDesc')}</EmptyDescription>
                </EmptyHeader>
              </Empty>
            )}
          </CardContent>
          ...
          {trips?.length ? (
            <div className="w-full space-y-4">
              <SearchInput
                ...
                placeholder={t('details.searchTrip')}
                value={query}
              />
              <div className="overflow-hidden rounded-md border">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead>{tCommon('table.name')}</TableHead>
                      <TableHead>{tCommon('table.departure')}</TableHead>
                      <TableHead>{tCommon('table.arrival')}</TableHead>
                      <TableHead>{tCommon('status.label')}</TableHead>
                      ...
                      <TableRow key={trip.id}>
                        ...
                        <TableCell className="flex gap-1">
                          <Button asChild size="sm" variant="outline">
                            <Link href={`/trips/${trip.id}`}>{tCommon('view')}</Link>
                          </Button>
                        </TableCell>
                      </TableRow>
                    ))}
