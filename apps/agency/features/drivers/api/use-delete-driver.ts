...
type ResponseType = InferResponseType<(typeof client.api.agency.drivers)[':identifier']['$delete']>;
...
export const useDeleteDriver = (options?: { onSuccess?: () => void; onError?: () => void }) => {
  ...
  return useMutation<ResponseType, Error, string>({
    mutationFn: async (identifier) => {
      const response = await client.api.agency.drivers[':identifier'].$delete({
        param: { identifier },
      });
      if (!response.ok) {
