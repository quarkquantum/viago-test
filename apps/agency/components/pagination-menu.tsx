...
export const PaginationMenu: React.FC<PaginationMenuProps> = ({
  ...
  const getPages = (p: number, tPages: number): (number | string)[] => {
    if (tPages <= 5) {
      return [...new Array(tPages)].map((_, i) => i + 1);
    }
  ...
  return (
    <div className="flex flex-col items-center justify-between gap-3 md:flex-row">
      ...
      <div className="whitespace-nowrap text-muted-foreground text-sm">
        {total > 0 ? t('pagination.showing', { start, end, total }) : t('pagination.noResults')}
      </div>
      ...
      <Pagination>
        <PaginationContent>
          ...
          {getPages(page, totalPages).map((p, i) =>
            p === '...' ? (
              <PaginationItem key={`dots-${i}-${p}`}>
                <PaginationEllipsis label={t('pagination.morePages')} />
              </PaginationItem>
            ) : (
              <PaginationItem key={p}>
                <PaginationLink
                  className={p === page ? 'border bg-muted font-bold' : 'font-light'}
                  href="#"
                  onClick={(e) => {
                    e.preventDefault();
      ...
      <div className="flex flex-row items-center justify-between gap-2">
        <span className="whitespace-nowrap pr-2 text-muted-foreground text-sm">{t('pagination.rowsPerPage')}</span>
