...
type ResponseType = InferResponseType<(typeof client.api.cashier.trips)[':identifier']['$get'], 200>;
export type Trip = ResponseType;
export type Station = Trip['stations'][number];
export type Booking = Trip['bookings'][number];

export const useGetTrip = (identifier: string) =>
  useQuery<ResponseType, Error>({
    enabled: Boolean(identifier),
    queryFn: async () => {
      const response = await client.api.cashier.trips[':identifier'].$get({
        param: { identifier },
      });
      if (!response.ok) {
