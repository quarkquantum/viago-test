...
export const OTPForm = ({ ...props }: React.ComponentProps<typeof Card>) => {
  ...
  const formSchema = useMemo(
    () =>
      z.object({
        otp: z.string().length(6, t('otpLengthError')),
      }),
    [t]
  );
  ...
  const onSubmit = async (data: z.infer<typeof formSchema>) => {
    setLoading(true);
    try {
      const { data: result, error } = await cashierAuthClient.twoFactor.verifyOtp({
        code: data.otp,
      });

      if (error) {
        toast.error(error.message || t('failedToVerifyOtp'));
        return;
      }
      ...
      console.error(error);
      toast.error(t('failedToVerifyOtp'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResend = async (e: React.MouseEvent) => {
    ...
    setResending(true);
    try {
      const { data: result } = await cashierAuthClient.twoFactor.sendOtp();
      if (result) {
              ...
              render={({ field, fieldState }) => (
                <Field data-invalid={fieldState.invalid}>
                  ...
                  {fieldState.invalid ? (
                    ...
                    <FieldDescription>{t('enterCodeHint')}</FieldDescription>
                  )}
                </Field>
              )}
            />
