...
function Header({ title, back, actions }: { title: string; back?: boolean | (() => void); actions?: ScreenAction[] }) {
  ...
  const canRenderBack = typeof back === 'function' || (Boolean(back) && Boolean(navigation?.canGoBack?.()));
  ...
  return (
    <View style={[styles.header, { paddingTop: insets.top }]}>
      <Animated.View style={[styles.headerRow, headerAnimatedStyle]}>
        {canRenderBack && <BackButton onPress={typeof back === 'function' ? back : undefined} />}
        <Title>{title}</Title>
...
function BackButton({ onPress }: { onPress?: () => void }) {
  const navigation = useContext(NavigationContext);
  const { scrollY } = useScreenContext();
  const canHandleBack = Boolean(onPress) || Boolean(navigation?.canGoBack?.());
  ...
  return (
    <Animated.View style={[styles.iconButton, styles.iconButtonSize, scaleStyle]}>
      <Pressable
        disabled={!canHandleBack}
        hitSlop={8}
        onPress={handlePress}
        style={({ pressed }) => [styles.iconButtonInner, !canHandleBack && styles.disabled, pressed && styles.pressed]}
      >
