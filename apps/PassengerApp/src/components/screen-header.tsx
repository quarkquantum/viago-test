...
type CollapsibleNavHeaderProps = {
  ...
  onGoBackPressed?: () => undefined | undefined;
  value?: string;
...
const CollapsibleNavHeader: React.FC<CollapsibleNavHeaderProps> = ({
  ...
  const renderAction = (action: Action, index: number) => {
    switch (action.type) {
      case 'avatar': {
        return (
          <TouchableOpacity key={index} onPress={action.onPress} style={[styles.actionButton, styles.avatarButton]}>
            {action.source ? (
              <Avatar.Image size={32} source={{ uri: action.source }} style={styles.avatar} />
            ) : (
              <View style={[styles.avatar, { backgroundColor: action.backgroundColor || '#FF5A5F' }]}>
      ...
      case 'icon': {
        return (
          <TouchableOpacity
            ...
            style={[styles.actionButton, { opacity: action.disabled ? 0.5 : 1 }]}
          >
          ...
          <View style={styles.titleContainer}>
            ...
            <Animated.Text
              ...
              style={[
                ...
                { color: titleColor },
                titleAnimatedStyle,
                { fontSize: titleAnimatedStyle.fontSize },
              ]}
...
type CollapsibleHeaderWrapperProps = {
  title?: string;
  actions?: Action[];
