...
export const RouteCard = ({ route, onPress }: Props) => {
  ...
  const seatColor = isSoldOut ? Colors.DESTRUCTIVE : route.seats.available > 5 ? Colors.PRIMARY : Colors.WARNING;

  const handlePress = () => {
    if (onPress) {
      onPress();
    } else {
      // Default: navigate to Reservation with route details
      navigation.navigate('Reservation', {
        busId: route.bus.licensePlate,
        busSeats: route.bus.seats,
        fromStationId: route.from.id,
        fromStationName: route.from.name,
        price: route.price,
        seatReservationType: route.bus.seatReservationType,
        toStationId: route.to.id,
        toStationName: route.to.name,
        tripId: route.trip.id,
      });
    }
  };

  return (
    <TouchableOpacity disabled={isSoldOut} onPress={handlePress} style={styles.card}>
      {/* Header */}
      ...
      <View style={styles.footer}>
        <View style={styles.details}>
          ...
          {/* Seats */}
          <View style={styles.detailItem}>
            <Users color={seatColor} size={15} />
            <Text style={[styles.detailText, { color: seatColor }]}>
              {route.seats.reserved} {t('screens.rides.details.reserved')} • {route.seats.available}{' '}
              {t('screens.rides.details.available')}
            </Text>
          </View>
