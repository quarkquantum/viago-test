...
import { NewBus } from '@/components/bus/new-bus';
...
const HEADERS_KEYS = [
  ...
  'fields.currentTrip',
  'table.status',
  'trips:fields.seatReservationType',
  'table.action',
];
...
export const Buses = () => {
  ...
  const filterList = [
    {
      label: tt('fields.seatReservationType'),
      options: [
        {
          label: tc('status.all'),
          value: undefined,
        ...
        {
          label: tt('fields.seatReservation'),
          value: BusSeatPolicy.NUMBERED,
        },
        {
          label: tt('fields.noSeatReservation'),
    ...
    },
    {
      ...
      options: [
        ...
        {
          label: tc('status.maintenance'),
          value: BusStatus.MAINTENANCE,
        },
        {
          label: tc('status.breakdown'),
          value: BusStatus.BREAKDOWN,
        },
        {
          label: tc('status.outOfService'),
          value: BusStatus.OUT_OF_SERVICE,
        },
        {
          label: tc('status.toReplace'),
              ...
              <TableHeader>
                <TableRow>
                  {HEADERS_KEYS.map((key) => (
                    <TableHead key={key}>
                      {key.startsWith('table.') ? tc(key) : key.startsWith('trips:') ? tt(key.split(':')[1]) : t(key)}
                    </TableHead>
                  ))}
                </TableRow>
              </TableHeader>
                  ...
                  <TableRow key={bus.id}>
                    ...
                    <TableCell>
                      {bus.trips && bus.trips.length > 0 ? (
                        <Button asChild size="sm" variant={'link'}>
                          <Link href={`/trips/${bus.trips[0].id}`}>{bus.trips[0].name}</Link>
                        </Button>
                      ) : (
                        '-'
                      )}
                    ...
                    <TableCell>
                      <Button asChild size="sm" variant="outline">
                        <Link href={`/buses/${bus.licensePlate}`}>{tc('view')}</Link>
                      </Button>
                    </TableCell>
                  </TableRow>
                ))}
