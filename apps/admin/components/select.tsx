...
export const Select = ({
  ...
  const finalSearchPlaceholder = searchPlaceholder || t('select.searchPlaceholder');
  const finalEmptyMessage = emptyMessage || t('select.emptyMessage');
    ...
    <Popover onOpenChange={setOpen} open={open}>
      ...
      <PopoverContent className="h-fit w-(--radix-popover-trigger-width) p-0">
        <Command shouldFilter={!onSearchChange}>
          {withSearch && (
            <CommandInput className="h-9" onValueChange={onSearchChange} placeholder={finalSearchPlaceholder} />
          )}
