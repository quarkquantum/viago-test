...
const formSchema = z.object({
  agencyId: z.string().min(1, { message: 'Company of Transport is required.' }),
...
export const NewAgencyManagerForm = ({ setOpen }: { setOpen: (open: boolean) => void }) => {
  const t = useTranslations();
  const createManager = useCreateAgencyManager({
    onSuccess: () => {
      form.reset();
      setOpen(false);
    },
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <Controller
          ...
          render={({ field, fieldState }) => (
            <Field data-invalid={fieldState.invalid}>
              <FieldLabel>{t('agencyManagers.create.agencyLabel')}</FieldLabel>
              <Select
                emptyMessage={isLoadingAgency ? t('common.loading') : t('common.select.emptyMessage')}
                onSearchChange={setAgencyQuery}
                onValueChange={field.onChange}
                options={agencies}
                placeholder={t('agencyManagers.create.selectAgency')}
                searchPlaceholder={t('agencyManagers.create.searchAgency')}
                value={field.value}
              />
          ...
          <Controller
            ...
            render={({ field, fieldState }) => (
              <Field data-invalid={fieldState.invalid}>
                ...
                <Input {...field} autoComplete="off" placeholder={t('common.table.firstName')} />
                {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
              </Field>
            )}
          />
          <Controller
            ...
            render={({ field, fieldState }) => (
              <Field data-invalid={fieldState.invalid}>
                ...
                <Input {...field} autoComplete="off" placeholder={t('common.table.lastName')} />
                {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
              </Field>
            )}
          />
        ...
        <Controller
          ...
          render={({ field, fieldState }) => (
            <Field data-invalid={fieldState.invalid}>
              ...
              <Input {...field} autoComplete="off" placeholder={t('common.table.email')} type="email" />
              {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
            </Field>
          )}
        />
        ...
        <Controller
          ...
          render={({ field, fieldState }) => (
            <Field data-invalid={fieldState.invalid}>
              ...
              <Input {...field} autoComplete="off" placeholder={t('common.table.phoneNumber')} />
              {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
            </Field>
          )}
        />
      ...
      <DialogFooter className="mt-6">
        ...
        <Button disabled={!isValid || createManager.isPending} onClick={handleSubmit(onSubmit)} type="submit">
          {createManager.isPending ? t('common.loading') : t('agencyManagers.create.submit')}
        </Button>
      </DialogFooter>
    </form>
  );
