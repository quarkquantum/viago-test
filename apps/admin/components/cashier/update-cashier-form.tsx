...
import type { Agency, Cashier } from '@/features/cashiers/api/use-get-cashier';

type CashierUser = Cashier['user'];
...
export const UpdateCashierForm = ({
  id,
  cashier,
  agency,
  setOpen,
  ...
  const form = useForm<FormValues>({
    defaultValues: {
      ...
      phoneNumber: cashier?.profile?.phoneNumber ?? '',
    },
  ...
  }, [cashier, agency, reset]);

  const agencies = [
    ...(agency && !agencyList?.data.some((a: Agency) => a.id === agency.id) ? [agency] : []),
    ...(agencyList?.data ?? []),
  ].map((a: Agency) => ({
    label: a.name,
    value: a.id,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <DialogFooter className="mt-6">
        ...
        <Button
          disabled={!(isDirty && isValid) || updateCashier.isPending}
          onClick={handleSubmit(onSubmit)}
          type="submit"
        >
          {updateCashier.isPending ? t('common.updating') : t('cashiers.edit.submit')}
        </Button>
      </DialogFooter>
    </form>
  );
